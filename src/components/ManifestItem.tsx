...
const resolvePublic = (path: string) => {
...
function ManifestItemCardInner({item, onOpen}: Props) {
    const src = useMemo(() => {
        const rel = `/${[item.folder, item.file].filter(Boolean).join('/')}`;
        return resolvePublic(rel);
    ...
    return (
        <div style={{width: 160, textAlign: 'center', margin: 12, cursor: 'pointer'}} onClick={() => onOpen?.(item)}>
            {type === 'image' &&
                <img src={src} alt={item.name} style={{width: 128, height: 128, borderRadius: '50%'}}/>}
            {type === 'video' && (
                <div style={{
                    width: 128,
                    height: 128,
