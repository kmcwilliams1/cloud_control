...
export default function ResourcesPopup({ visible, provider, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const url = urlPath ? (urlPath.startsWith('/') ? `${base}${urlPath}` : `${base}/${urlPath}`) : null;
  ...
  return (
    <div role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, display: 'grid', placeItems: 'center', background: 'rgba(0,0,0,0.35)', zIndex: 1200, padding: 24 }}>
      <div style={{ width: 'min(1100px, 96%)', maxHeight: '90vh', overflowY: 'auto', background: '#fff', borderRadius: 10, border: '1px solid #e6e6e6', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', padding: 20 }}>
