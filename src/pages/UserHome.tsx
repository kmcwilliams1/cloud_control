...
export default function GuestHome(): JSX.Element {
  ...
  const allowedForUser = (it: MI) => {
    ...
    if (Array.isArray(anyIt.roles) && anyIt.roles.length > 0) {
      // roles array present -> require intersection or admin always allowed
    ...
    if (typeof anyIt.access === 'string') {
      ...
      // allow matching role (e.g. 'user')
      return access === role || role === 'admin';
    }
    return true; // no access metadata => visible
  };

  // derive filtered items once
  ...
  return (
    <div style={{ padding: 5, minHeight: '100vh', fontFamily: 'Montserrat, Arial' }}>
      ...
      <a href="/account" style={{ textAlign: 'right', position: 'absolute', top: 10, right: 10 }}>
        <img src={'/settings-glyph-black-icon-png_292947.jpg'} alt={'Settings'} style={{ width: 40, height: 40, cursor: 'pointer' }} />
      </a>
        ...
        {providerOrder.map((p) => (
          <section key={p} style={{ padding: 5, borderRight: '1px solid #000000', textAlign: 'center' }}>
            <div style={{
              ...
              {p === 'aws' ? (
                <button onClick={() => { setResourcesProvider('aws'); setResourcesVisible(true); }}>
                  Resources
                </button>
              ) : (
                <button onClick={() => {
                ...
                return (
                  <>
                    {graphsForP.map((it: MI) => (
                      <a key={`${it.folder}/${it.file}`} href={`/${it.folder}/${it.file}`} onClick={(e) => { e.preventDefault(); openPicker(it); }}>
                        <img src={`/${it.folder}/${it.file}`} alt={it.name} style={{ width: 300, flexWrap: 'wrap' }} />
                      </a>
                    ))}
                    <div style={{ height: 12 }} />
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center', alignItems: 'flex-start' }}>
                      {notGraphsForP.map((it: MI) => (
                        <a key={`${it.folder}/${it.file}`} href={`/${it.folder}/${it.file}`} onClick={(e) => { e.preventDefault(); openPicker(it); }} style={{ margin: 4, width: 130, display: 'flex', flexDirection: 'column', alignItems: 'center', textDecoration: 'none', color: 'inherit' }}>
                          <p style={{ margin: '4px 0', width: 130, textAlign: 'center', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{it.name}</p>
                          <img src={`/${it.folder}/${it.file}`} alt={it.name} style={{ width: 130, height: 'auto', display: 'block' }} />
                        </a>
                      ))}
                    </div>
                  </>
